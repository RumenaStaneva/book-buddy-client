...
type AddScreenTimeProps = {
  setIsOpenAddScreenTime: (isOpen: boolean) => void;
  handleCloseModal: () => void;
  previousElement: Element | null;
};

const AddScreenTimeModal = ({
  setIsOpenAddScreenTime,
  handleCloseModal,
  previousElement,
}: AddScreenTimeProps) => {
  const [screenTimeData, setScreenTimeData] = useState(
    Array(7).fill({ date: "", time: "00:00" })
  );

  const [datesFromLastWeek, setDatesFromLastWeek] = useState<Date[]>([]);
  ...
  const dispatchReadingTime = useAppDispatch();
  const daysOfWeek = [
    "Monday",
    "Tuesday",
    "Wednesday",
  ...
  useEffect(() => {
    const getWeekDates = async () => {
      ...
      try {
        ...
        const datesFromLastWeek = eachDayOfInterval({
          start: lastWeekStart,
          end: lastWeekEnd,
        });
        ...
        const response = await fetch(
          `${process.env.REACT_APP_LOCAL_HOST}/time-swap/week-dates`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          throw new window.Error(
            "Error fetching week dates" + response.statusText
          );
        }
      } catch (error: any) {
  ...
  const convertToSeconds = (time: string, index: number) => {
    if (time === "00:00") {
  ...
  const validateInputs = () => {
    ...
    const invalidInputs = screenTimeData.reduce(
      (invalidInputs, item, index) => {
        if (item.time === "00:00") {
  ...
  const checkScreenTime = async (e: SyntheticEvent) => {
  ...
  const saveScreenTime = async () => {
    setIsLoading(true);
    try {
      const formattedDates = datesFromLastWeek.map((date) =>
      ...
      const response = await fetch(
        `${process.env.REACT_APP_LOCAL_HOST}/time-swap/save-time`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      dispatchReadingTime(
        fetchReadingTimeForTheWeek({ user, dataRange: "Current week" })
      );
      handleCloseModal();
    } catch (error: any) {
      setIsLoading(false);
                    ...
                    <div key={index} className="input-field__container">
                      ...
                      <div
                        className={`input-field ${
                          showConfirmationDialog && "confirmation-dialog-shown"
                        } ${invalidInputs.includes(index) ? "error" : ""}`}
                      >
                        <Cleave
                          options={{
                            time: true,
